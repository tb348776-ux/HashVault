  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-b ${effortColor} opacity-20`}
        />
        ...
        <div className="absolute top-0 left-0 right-0 bg-black/60 backdrop-blur-sm py-1.5 px-2 border-b border-white/10 flex items-center justify-between">
          ...
          <span
            className={`text-[8px] font-mono font-bold px-1 py-0.5 rounded ${isSolo ? "bg-green-500/30 text-green-400" : "bg-pink-500/30 text-pink-400"}`}
          >
            {isSolo ? "SOLO" : "POOL"}
          </span>
        </div>
...
const PendingBlock = ({
  ...
  const estimatedSeconds =
    difficulty && poolHashRate ? Math.floor(difficulty / poolHashRate) : 0;
      ...
      <div className="mt-3 md:mt-4 grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 p-3 md:p-4 bg-black/40 rounded-lg border border-gray-800">
        <div className="text-center">
          <div className="text-lg font-mono font-bold text-cyan-400">
            {blocks?.length && blocks[0].height
              ? formatNumber(blocks[0].height)
              : "-"}
          </div>
          <div className="text-[10px] uppercase tracking-wider text-gray-500">
            Latest Height

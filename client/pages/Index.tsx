...
const fetchPoolStats = async () => {
  const response = await fetch(
    "https://api.hashvault.pro/v3/monero/pool/stats",
  );
...
const fetchHashrateChart = async () => {
  const response = await fetch(
    "https://api.hashvault.pro/v3/monero/pool/chart/hashrate/allWorkers",
  );
...
const fetchBlocks = async () => {
  const response = await fetch(
    "https://api.hashvault.pro/v3/monero/pool/blocks",
  );
...
const ScanLines = () => (
  <div className="pointer-events-none fixed inset-0 z-50 overflow-hidden opacity-10">
    <div
      className="absolute inset-0 bg-gradient-to-b from-transparent via-cyan-500/5 to-transparent animate-scan"
...
const StatCard = ({
  ...
  return (
    <div
      className={`relative ${c.bg} ${c.border} border rounded-lg p-4 backdrop-blur-sm
    ...
    >
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-2">
          ...
          <Icon
            className={`w-4 h-4 ${c.text} opacity-60 group-hover:opacity-100 transition-opacity`}
          />
        </div>
        {loading ? (
          ...
          <div
            className={`text-xl md:text-2xl font-bold font-mono ${c.text} truncate`}
          >
            {value}
          </div>
        )}
      ...
      <div
        className={`absolute top-0 left-0 w-2 h-2 border-t border-l ${c.border}`}
      />
      <div
        className={`absolute top-0 right-0 w-2 h-2 border-t border-r ${c.border}`}
      />
      <div
        className={`absolute bottom-0 left-0 w-2 h-2 border-b border-l ${c.border}`}
      />
      <div
        className={`absolute bottom-0 right-0 w-2 h-2 border-b border-r ${c.border}`}
      />
    </div>
  );
};
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span
          className={`text-sm font-mono font-bold ${isOver100 ? "text-orange-400 animate-pulse" : c.text}`}
        >
          {(value || 0).toFixed(2)}%
        </span>
      </div>
...
const CyberPanel = ({
  ...
  return (
    <div
      className={`bg-black/40 ${c.border} border rounded-xl p-4 md:p-6 backdrop-blur-md relative overflow-hidden`}
    >
      <div
        className={`absolute top-0 left-0 right-0 h-px ${c.line} opacity-50`}
      />
      <div className="flex items-center gap-3 mb-4">
        {Icon && <Icon className={`w-5 h-5 ${c.text}`} />}
        <h2 className={`text-sm uppercase tracking-widest font-mono ${c.text}`}>
          {title}
        </h2>
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-gray-800/50 last:border-0">
      ...
      <div className="text-right">
        ...
        {change !== undefined && (
          <span
            className={`ml-2 text-xs ${isPositive ? "text-green-400" : "text-red-400"}`}
          >
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0f] text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => refetch()}
            className="mt-4 px-4 py-2 bg-cyan-500/20 border border-cyan-500/50 rounded-lg text-cyan-400 font-mono text-sm hover:bg-cyan-500/30 transition-colors"
          >
            Retry
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white overflow-x-hidden">
      ...
      <div className="relative z-10 p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-3">
              <div
                className="w-12 h-12 rounded-lg bg-orange-500/20 border border-orange-500/50 flex items-center justify-center
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 font-mono mt-1">
                  MONERO MINING PROTOCOL // LIVE FEED
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div
              className={`w-2 h-2 rounded-full ${pulse ? "bg-green-400 shadow-lg shadow-green-500/50" : "bg-green-500/50"} transition-all`}
            />
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-mono text-cyan-300 truncate block">
                      {formatNumber(network?.difficulty)}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-mono text-cyan-400">
                    {formatNumber(template?.difficulty)}
                  </span>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="text-lg font-bold font-mono text-pink-400">
                      {formatNumber(pool?.totalBlocksFound)}
                    </span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500 font-mono block">
                    ...
                    <span className="text-lg font-mono text-red-400">
                      {formatNumber(pool?.orphans)}
                    </span>
                  </div>
                </div>
                <EffortMeter
                  label="Current Effort"
                  value={pool?.currentEffort}
                  color="pink"
                />
                <EffortMeter
                  label="Overall Effort"
                  value={pool?.overallEffort}
                  color="orange"
                />
                <div className="flex justify-between items-center pt-2 border-t border-gray-800">
                  ...
                  <span className="text-sm font-mono text-pink-300">
                    {Math.floor((pool?.pplnsWindowTime || 0) / 60)}:
                    {String((pool?.pplnsWindowTime || 0) % 60).padStart(2, "0")}
                  </span>
                </div>
                <div className="bg-gray-900/50 rounded-lg p-3 border border-pink-500/10">
                  ...
                  <code className="text-xs font-mono text-pink-300">
                    {pool?.lastFoundBlock?.hash?.slice(0, 8)}...
                    {pool?.lastFoundBlock?.hash?.slice(-8)}
                  </code>
                  <div className="flex justify-between mt-2 text-xs">
                    <span className="text-gray-500">
                      Height: {formatNumber(pool?.lastFoundBlock?.height)}
                    </span>
                    <span className="text-pink-400">
                      {formatXMR(pool?.lastFoundBlock?.value)} XMR
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="text-lg font-bold font-mono text-green-400">
                      {formatNumber(solo?.totalBlocksFound)}
                    </span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500 font-mono block">
                    ...
                    <span className="text-lg font-mono text-red-400">
                      {formatNumber(solo?.orphans)}
                    </span>
                  </div>
                </div>
                <EffortMeter
                  label="Current Effort"
                  value={solo?.currentEffort}
                  color="green"
                />
                <EffortMeter
                  label="Overall Effort"
                  value={solo?.overallEffort}
                  color="cyan"
                />
                ...
                <div className="bg-gray-900/50 rounded-lg p-3 border border-green-500/10">
                  ...
                  <code className="text-xs font-mono text-green-300">
                    {solo?.lastFoundBlock?.hash?.slice(0, 8)}...
                    {solo?.lastFoundBlock?.hash?.slice(-8)}
                  </code>
                  <div className="flex justify-between mt-2 text-xs">
                    <span className="text-gray-500">
                      Height: {formatNumber(solo?.lastFoundBlock?.height)}
                    </span>
                    <span className="text-green-400">
                      {formatXMR(solo?.lastFoundBlock?.value)} XMR
                    </span>
                  </div>
                </div>
              </div>
        ...
        {/* Hash Rate Chart */}
        <CyberPanel
          title="Pool Hash Rate Telemetry"
          icon={Activity}
          color="cyan"
        >
          <div className="h-64">
            ...
            {isLoading ? (
              ...
              <>
                <div className="space-y-1">
                  <MarketItem
                    label="XMR/BTC"
                    value={(market?.price_btc || 0).toFixed(8)}
                  />
                  <MarketItem
                    label="XMR/USD"
                    value={`$${(market?.price_usd || 0).toFixed(2)}`}
                    change={market?.percent_change_24h}
                  />
                  <MarketItem
                    label="XMR/EUR"
                    value={`€${(market?.price_eur || 0).toFixed(2)}`}
                  />
                  <MarketItem
                    label="Market Cap"
                    value={`${formatNumber(market?.market_cap_btc)} BTC`}
                  />
                  <MarketItem
                    label="24h Volume"
                    value={`${formatNumber(market?.["24h_volume_btc"])} BTC`}
                  />
                  <MarketItem
                    label="7 Day"
                    value=""
                    change={market?.percent_change_7d}
                  />
                </div>
          ...
          <CyberPanel title="Pool Configuration" icon={Gauge} color="orange">
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center py-2 border-b border-gray-800/50">
                ...
                <span className="text-sm font-mono text-orange-400">
                  {formatXMR(poolData?.config?.payout?.minimum)} XMR
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-800/50">
                <span className="text-xs text-gray-500 font-mono">
              ...
              <div className="flex justify-between items-center py-2 border-b border-gray-800/50">
                ...
                <span className="text-sm font-mono text-orange-400">
                  {poolData?.config?.maturity_depth || 30} blocks
                </span>
              </div>
        ...
        <div className="text-center py-6 border-t border-gray-800/50">
          ...
          <div className="flex justify-center gap-4 mt-3">
            <a
              href="https://monero.hashvault.pro"
              target="_blank"
